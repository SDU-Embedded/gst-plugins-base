 ...
 *
 * gstaudioeventemitter.h: base class for audio visualisation elements
 *
 ...
 */
/**
 * SECTION:gstaudioeventemitter
 * @title: GstAudioEventEmitter
...
#include "gstaudioeventemitter.h"
#include "pbutils-enumtypes.h"

GST_DEBUG_CATEGORY_STATIC (audio_event_emitter_debug);
#define GST_CAT_DEFAULT (audio_event_emitter_debug)
#define GST_TYPE_AUDIO_EVENT_EMITTER_SHADER 0
#define DEFAULT_SHADER GST_AUDIO_EVENT_EMITTER_SHADER_FADE
...
static void gst_audio_event_emitter_class_init (GstAudioEventEmitterClass *
    klass);
static void gst_audio_event_emitter_init (GstAudioEventEmitter * scope,
    GstAudioEventEmitterClass * g_class);
static void gst_audio_event_emitter_set_property (GObject * object,
    guint prop_id, const GValue * value, GParamSpec * pspec);
static void gst_audio_event_emitter_get_property (GObject * object,
    guint prop_id, GValue * value, GParamSpec * pspec);
static void gst_audio_event_emitter_dispose (GObject * object);

static gboolean gst_audio_event_emitter_src_negotiate (GstAudioEventEmitter *
    scope);
static gboolean gst_audio_event_emitter_src_setcaps (GstAudioEventEmitter *
    scope, GstCaps * caps);
static gboolean gst_audio_event_emitter_sink_setcaps (GstAudioEventEmitter *
    scope, GstCaps * caps);

static GstFlowReturn gst_audio_event_emitter_chain (GstPad * pad,
    GstObject * parent, GstBuffer * buffer);

static gboolean gst_audio_event_emitter_src_event (GstPad * pad,
    GstObject * parent, GstEvent * event);
static gboolean gst_audio_event_emitter_sink_event (GstPad * pad,
    GstObject * parent, GstEvent * event);

static gboolean gst_audio_event_emitter_src_query (GstPad * pad,
    GstObject * parent, GstQuery * query);

static GstStateChangeReturn gst_audio_event_emitter_change_state (GstElement *
    element, GstStateChange transition);

static gboolean gst_audio_event_emitter_do_bufferpool (GstAudioEventEmitter *
    scope, GstCaps * outcaps);

static gboolean
default_decide_allocation (GstAudioEventEmitter * scope, GstQuery * query);

struct _GstAudioEventEmitterPrivate
{
  gboolean negotiated;
  ...
  GstAudioEventEmitterShader shader_type;
  GstAudioEventEmitterShaderFunc shader;
...
static void
shader_fade (GstAudioEventEmitter * scope, const GstVideoFrame * sframe,
...
static void
shader_fade_and_move_up (GstAudioEventEmitter * scope,
...
static void
shader_fade_and_move_down (GstAudioEventEmitter * scope,
    const GstVideoFrame * sframe, GstVideoFrame * dframe)
{
  guint i, j;
  guint r = (scope->priv->shade_amount >> 16) & 0xff;
...
static void
shader_fade_and_move_left (GstAudioEventEmitter * scope,
    const GstVideoFrame * sframe, GstVideoFrame * dframe)
{
  guint i, j;
  guint r = (scope->priv->shade_amount >> 16) & 0xff;
...
static void
shader_fade_and_move_right (GstAudioEventEmitter * scope,
    const GstVideoFrame * sframe, GstVideoFrame * dframe)
{
  guint i, j;
  guint r = (scope->priv->shade_amount >> 16) & 0xff;
...
static void
shader_fade_and_move_horiz_out (GstAudioEventEmitter * scope,
    const GstVideoFrame * sframe, GstVideoFrame * dframe)
{
  guint i, j;
  guint r = (scope->priv->shade_amount >> 16) & 0xff;
...
static void
shader_fade_and_move_horiz_in (GstAudioEventEmitter * scope,
    const GstVideoFrame * sframe, GstVideoFrame * dframe)
{
  guint i, j;
  guint r = (scope->priv->shade_amount >> 16) & 0xff;
...
static void
shader_fade_and_move_vert_out (GstAudioEventEmitter * scope,
    const GstVideoFrame * sframe, GstVideoFrame * dframe)
{
  guint i, j;
  guint r = (scope->priv->shade_amount >> 16) & 0xff;
...
static void
shader_fade_and_move_vert_in (GstAudioEventEmitter * scope,
    const GstVideoFrame * sframe, GstVideoFrame * dframe)
{
  guint i, j;
  guint r = (scope->priv->shade_amount >> 16) & 0xff;
...
static void
gst_audio_event_emitter_change_shader (GstAudioEventEmitter * scope)
{
  switch (scope->priv->shader_type) {
    case GST_AUDIO_EVENT_EMITTER_SHADER_NONE:
      scope->priv->shader = NULL;
      break;
    case GST_AUDIO_EVENT_EMITTER_SHADER_FADE:
      scope->priv->shader = shader_fade;
      break;
    case GST_AUDIO_EVENT_EMITTER_SHADER_FADE_AND_MOVE_UP:
      scope->priv->shader = shader_fade_and_move_up;
      break;
    case GST_AUDIO_EVENT_EMITTER_SHADER_FADE_AND_MOVE_DOWN:
      scope->priv->shader = shader_fade_and_move_down;
      break;
    case GST_AUDIO_EVENT_EMITTER_SHADER_FADE_AND_MOVE_LEFT:
      scope->priv->shader = shader_fade_and_move_left;
      break;
    case GST_AUDIO_EVENT_EMITTER_SHADER_FADE_AND_MOVE_RIGHT:
      scope->priv->shader = shader_fade_and_move_right;
      break;
    case GST_AUDIO_EVENT_EMITTER_SHADER_FADE_AND_MOVE_HORIZ_OUT:
      scope->priv->shader = shader_fade_and_move_horiz_out;
      break;
    case GST_AUDIO_EVENT_EMITTER_SHADER_FADE_AND_MOVE_HORIZ_IN:
      scope->priv->shader = shader_fade_and_move_horiz_in;
      break;
    case GST_AUDIO_EVENT_EMITTER_SHADER_FADE_AND_MOVE_VERT_OUT:
      scope->priv->shader = shader_fade_and_move_vert_out;
      break;
    case GST_AUDIO_EVENT_EMITTER_SHADER_FADE_AND_MOVE_VERT_IN:
...
GType
gst_audio_event_emitter_get_type (void)
{
  static volatile gsize audio_event_emitter_type = 0;

  if (g_once_init_enter (&audio_event_emitter_type)) {
    static const GTypeInfo audio_event_emitter_info = {
      sizeof (GstAudioEventEmitterClass),
      NULL,
      NULL,
      (GClassInitFunc) gst_audio_event_emitter_class_init,
      NULL,
      NULL,
      sizeof (GstAudioEventEmitter),
      0,
      (GInstanceInitFunc) gst_audio_event_emitter_init,
    };
    GType _type;

    /* TODO: rename when exporting it as a library */
    _type = g_type_register_static (GST_TYPE_ELEMENT,
        "GstAudioEventEmitter", &audio_event_emitter_info,
        G_TYPE_FLAG_ABSTRACT);

    private_offset =
        g_type_add_instance_private (_type,
        sizeof (GstAudioEventEmitterPrivate));

    g_once_init_leave (&audio_event_emitter_type, _type);
  }
  return (GType) audio_event_emitter_type;
}

static inline GstAudioEventEmitterPrivate *
gst_audio_event_emitter_get_instance_private (GstAudioEventEmitter * self)
{
  return (G_STRUCT_MEMBER_P (self, private_offset));
}

static void
gst_audio_event_emitter_class_init (GstAudioEventEmitterClass * klass)
{
  ...
  GST_DEBUG_CATEGORY_INIT (audio_event_emitter_debug,
      "baseaudioeventemitter-libvisual", 0,
      "scope audio visualisation base class");

  gobject_class->set_property = gst_audio_event_emitter_set_property;
  gobject_class->get_property = gst_audio_event_emitter_get_property;
  gobject_class->dispose = gst_audio_event_emitter_dispose;

  element_class->change_state =
      GST_DEBUG_FUNCPTR (gst_audio_event_emitter_change_state);
  ...
  g_object_class_install_property (gobject_class, PROP_SHADER,
      g_param_spec_enum ("shader", "shader type",
          ...
          GST_TYPE_AUDIO_EVENT_EMITTER_SHADER, DEFAULT_SHADER,
...
static void
gst_audio_event_emitter_init (GstAudioEventEmitter * scope,
    GstAudioEventEmitterClass * g_class)
{
  GstPadTemplate *pad_template;

  scope->priv = gst_audio_event_emitter_get_instance_private (scope);
  ...
  gst_pad_set_chain_function (scope->priv->sinkpad,
      GST_DEBUG_FUNCPTR (gst_audio_event_emitter_chain));
  gst_pad_set_event_function (scope->priv->sinkpad,
      GST_DEBUG_FUNCPTR (gst_audio_event_emitter_sink_event));
  ...
  gst_pad_set_event_function (scope->priv->srcpad,
      GST_DEBUG_FUNCPTR (gst_audio_event_emitter_src_event));
  gst_pad_set_query_function (scope->priv->srcpad,
      GST_DEBUG_FUNCPTR (gst_audio_event_emitter_src_query));
  ...
  gst_audio_event_emitter_change_shader (scope);
...
static void
gst_audio_event_emitter_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec)
{
  GstAudioEventEmitter *scope = GST_AUDIO_EVENT_EMITTER (object);
...
static void
gst_audio_event_emitter_get_property (GObject * object, guint prop_id,
...
static void
gst_audio_event_emitter_dispose (GObject * object)
{
  GstAudioEventEmitter *scope = GST_AUDIO_EVENT_EMITTER (object);
...
static void
gst_audio_event_emitter_reset (GstAudioEventEmitter * scope)
{
...
static gboolean
gst_audio_event_emitter_sink_setcaps (GstAudioEventEmitter * scope,
    GstCaps * caps)
{
  GstAudioInfo info;
  ...
  if (!gst_audio_event_emitter_src_negotiate (scope)) {
    goto not_negotiated;
  }
...
static gboolean
gst_audio_event_emitter_src_setcaps (GstAudioEventEmitter * scope,
    GstCaps * caps)
{
  GstVideoInfo info;
  GstAudioEventEmitterClass *klass;
  gboolean res;

  if (!gst_video_info_from_caps (&info, caps))
    goto wrong_caps;

  klass = GST_AUDIO_EVENT_EMITTER_CLASS (G_OBJECT_GET_CLASS (scope));
  ...
  res = gst_audio_event_emitter_do_bufferpool (scope, caps);
  gst_caps_unref (caps);
...
static gboolean
gst_audio_event_emitter_src_negotiate (GstAudioEventEmitter * scope)
{
  ...
  ret = gst_audio_event_emitter_src_setcaps (scope, target);
...
static gboolean
gst_audio_event_emitter_set_allocation (GstAudioEventEmitter * scope,
    ...
    GstAllocationParams * params, GstQuery * query)
{
  ...
  GstQuery *oldquery;
  GstAudioEventEmitterPrivate *priv = scope->priv;
...
static gboolean
gst_audio_event_emitter_do_bufferpool (GstAudioEventEmitter * scope,
    GstCaps * outcaps)
{
  GstQuery *query;
  ...
  klass = GST_AUDIO_EVENT_EMITTER_GET_CLASS (scope);
  ...
  /* now store */
  result =
      gst_audio_event_emitter_set_allocation (scope, pool, allocator, &params,
      query);
...
default_prepare_output_buffer (GstAudioEventEmitter * scope,
    GstBuffer ** outbuf)
{
...
static GstFlowReturn
gst_audio_event_emitter_chain (GstPad * pad, GstObject * parent,
    GstBuffer * buffer)
{
  GstFlowReturn ret = GST_FLOW_OK;
  GstAudioEventEmitter *scope;
  GstAudioEventEmitterClass *klass;
  GstBuffer *inbuf;
  guint64 dist, ts;
  guint avail, sbpf;
  ...
  scope = GST_AUDIO_EVENT_EMITTER (parent);
  klass = GST_AUDIO_EVENT_EMITTER_CLASS (G_OBJECT_GET_CLASS (scope));
...
static gboolean
gst_audio_event_emitter_src_event (GstPad * pad, GstObject * parent,
    GstEvent * event)
{
  gboolean res;
  GstAudioEventEmitter *scope;
...
static gboolean
gst_audio_event_emitter_sink_event (GstPad * pad, GstObject * parent,
    GstEvent * event)
{
  gboolean res;
  GstAudioEventEmitter *scope;
  ...
  switch (GST_EVENT_TYPE (event)) {
    case GST_EVENT_CAPS:
    {
      ...
      res = gst_audio_event_emitter_sink_setcaps (scope, caps);
      gst_event_unref (event);
      break;
    }
    case GST_EVENT_FLUSH_STOP:
      gst_audio_event_emitter_reset (scope);
...
static gboolean
gst_audio_event_emitter_src_query (GstPad * pad, GstObject * parent,
    GstQuery * query)
{
  gboolean res = FALSE;
  GstAudioEventEmitter *scope;
...
gst_audio_event_emitter_change_state (GstElement * element,
    GstStateChange transition)
{
  GstStateChangeReturn ret;
  GstAudioEventEmitter *scope;

  scope = GST_AUDIO_EVENT_EMITTER (element);
  ...
  switch (transition) {
    case GST_STATE_CHANGE_PAUSED_TO_READY:
      gst_audio_event_emitter_set_allocation (scope, NULL, NULL, NULL, NULL);
      break;
